...
displays information pertaining to CPU, Task, Node, Resident Set Size (RSS) and Virtual Memory (VM).
You can tailor the output with the use of the **--fields=** option to specify the fields to be
shown.
...
required. The step portion will default to the lowest numbered (not batch, extern, etc) step running
if not specified, unless the --allsteps flag is set where not specifying a step will result in all
running steps to be displayed. **NOTE**: A step id of 'batch' will display the information about the
batch step. **NOTE**: A step id of 'extern' will display the information about the extern step. This
step is only available when using PrologFlags=contain
...
accounting fields look at the values for all the tasks of each step in a job and return the average,
maximum or minimum values for the job step.
...
>
> TresUsageInAve  
> Tres average usage in by all tasks in job. **NOTE**: If corresponding TresUsageInMaxTask is -1 the
> metric is node centric instead of task.
>
> <!-- -->
>
> TresUsageInMax  
> Tres maximum usage in by all tasks in job. **NOTE**: If corresponding TresUsageInMaxTask is -1 the
> metric is node centric instead of task.
>
> <!-- -->
>
...
>
> TresUsageOutAve  
> Tres average usage out by all tasks in job. **NOTE**: If corresponding TresUsageOutMaxTask is -1
> the metric is node centric instead of task.
>
> <!-- -->
>
> TresUsageOutMax  
> Tres maximum usage out by all tasks in job. **NOTE**: If corresponding TresUsageOutMaxTask is -1
> the metric is node centric instead of task.
>
> <!-- -->
>
...
Executing **sstat** sends a remote procedure call to **slurmctld**. If enough calls from **sstat**
...
**sstat** to the minimum necessary for the information you are trying to gather.

# ENVIRONMENT VARIABLES

Some **sstat** options may be set via environment variables. These environment variables, along with
...
**SLURM_DEBUG_FLAGS**  
Specify debug flags for sstat to use. See DebugFlags in the **slurm.conf**(5) man page for a full
